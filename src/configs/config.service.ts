...
export class ConfigService {
  ...
  get redisClusterMode(): 'nats_cluster' | 'aws_elasticache' | 'local_cluster' {
    const mode =
      ...
      this.envConfig['REDIS_CLUSTER_MODE'] ||
      '';
    if (
      mode &&
      ['nats_cluster', 'aws_elasticache', 'local_cluster'].includes(mode)
    ) {
  ...
  get redisClusterHost(): string {
    return (
      ...
      this.envConfig['REDIS_CLUSTER_HOST'] ||
      '127.0.0.1'
    );
  }
  ...
  get redisClusterPort(): number {
    return this.int(
      process.env.REDIS_CLUSTER_PORT || this.envConfig['REDIS_CLUSTER_PORT'],
      6000,
    );
  }
  ...
  get redisClusterSlaveRead(): string {
    ...
    const redisClusterSlaveRead =
      process.env.REDIS_CLUSTER_SLAVE_READ ||
      this.envConfig['REDIS_CLUSTER_SLAVE_READ'] ||
      '';
  ...
  get useRedisCluster(): boolean {
    return this.bool(
      process.env.USE_REDIS_CLUSTER || this.envConfig['USE_REDIS_CLUSTER'],
      false,
    );
  }
  ...
  get redisPort(): number {
    return this.int(
      process.env.REDIS_PORT || this.envConfig['REDIS_PORT'],
      6379,
    );
  }
  ...
  get isPrismaLogEnabled(): boolean {
    return this.bool(
      process.env.IS_PRISMA_LOG_ENABLED ||
        this.envConfig['IS_PRISMA_LOG_ENABLED'],
      true,
    );
  }
  get mongoDBUri(): string {
    return (
      process.env.MONGODB_URI || this.envConfig['MONGODB_URI'] || '127.0.0.1'
    );
  }
  get mongoDBDatabase(): string {
    return (
      process.env.MONGODB_DATABASE ||
      this.envConfig['MONGODB_DATABASE'] ||
      'mongo-local'
    );
  }
  get host(): string {
  ...
  get corsEnabled(): boolean {
    return this.bool(
      process.env.CORS_ENABLED || this.envConfig['CORS_ENABLED'],
      true,
    );
  }
  ...
  get isRedisCluster(): boolean {
    return this.bool(
      process.env.IS_REDIS_CLUSTER || this.envConfig['IS_REDIS_CLUSTER'],
      false,
    );
  }
  ...
  get isIntegrationTest(): boolean {
    return this.bool(
      process.env.IS_INTEGRATION_TESTING ||
        this.envConfig['IS_INTEGRATION_TESTING'],
      false,
    );
  }
  ...
  get sendGridApiKey(): string {
    return (
      process.env.SEND_GRID_API_KEY || this.envConfig['SEND_GRID_API_KEY'] || ''
    );
  }
  ...
  get twilioPhoneNumber(): string {
    return (
      ...
      this.envConfig['TWILIO_PHONE_NUMBER'] ||
      '+19124204933'
    );
  }
  get twilioAccountSid(): string {
    return (
      ...
      this.envConfig['TWILIO_ACCOUNT_SID'] ||
      ''
    );
  }
  get twilioAuthToken(): string {
    return (
      process.env.TWILIO_AUTH_TOKEN || this.envConfig['TWILIO_AUTH_TOKEN'] || ''
    );
  }
  ...
  get stripeSecretKey(): string {
    return (
      process.env.STRIPE_SECRET_KEY || this.envConfig['STRIPE_SECRET_KEY'] || ''
    );
  }

  get stripeCurrency(): string {
    return (
      process.env.STRIPE_CURRENCY || this.envConfig['STRIPE_CURRENCY'] || ''
    );
  }
  ...
  get stripeStatementDescriptor(): string {
    return (
      ...
      this.envConfig['STRIPE_STATEMENT_DESCRIPTOR'] ||
      ''
    );
  }
  ...
  get publishableKey(): string {
    return (
      ...
      this.envConfig['STRIPE_PUBLISHABLE_KEY'] ||
      ''
    );
  }
  ...
  get originDomain(): string {
    return (
      ...
      this.envConfig['ORIGIN_DOMAIN'] ||
      'dxjxo84zunoqc.cloudfront.net'
    );
  }
  ...
  get backendDomain(): string {
    return (
      ...
      this.envConfig['BACKEND_DOMAIN'] ||
      'https://dev.api.kinsend.io'
    );
  }

  get amplifyBrand(): string {
    return (
      process.env.AMPLIFY_BRAND || this.envConfig['AMPLIFY_BRAND'] || 'develop'
    );
  }
  ...
  get isTestModePaymentMonthly(): boolean {
    const result =
      process.env.IS_TEST_PAYMENT_MONTHLY ||
      this.envConfig['IS_TEST_PAYMENT_MONTHLY'] ||
      false;
  ...
  get priceStarterOldPlane(): number {
    const result =
      process.env.PRICE_STARTER_OLD_PLANE ||
      this.envConfig['PRICE_STARTER_OLD_PLANE'] ||
      1999;
    return Number(result);
  }
  ...
  get priceStarterPlane(): number {
    const result =
      process.env.PRICE_STARTER_PLANE ||
      this.envConfig['PRICE_STARTER_PLANE'] ||
      4999;
    return Number(result);
  }
  ...
  get priceGrowthPlane(): number {
    const result =
      process.env.PRICE_GROWTH_PLANE ||
      this.envConfig['PRICE_GROWTH_PLANE'] ||
      24999;
    return Number(result);
  }
  ...
  get priceHighVolumePlane(): number {
    const result =
      process.env.PRICE_HIGH_VOLUME_PLANE ||
      this.envConfig['PRICE_HIGH_VOLUME_PLANE'] ||
      49900;
    return Number(result);
  }
  ...
  get frontEndDomain(): string {
    return (
      ...
      this.envConfig['FE_DOMAIN'] ||
      'https://app.dev.kinsend.io'
    );
  }
